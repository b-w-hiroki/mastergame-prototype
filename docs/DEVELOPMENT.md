...
ポイ活のリテンション施策の定番ですが実装がゼロでした。0028 で計測基盤が入ったので、
**導入後に D1/D7 が動いたかを検証できる状態**で入れています。

| 要素 | 内容 |
|---|---|
| `service_today()` | **日付の境界をサーバ側で決める**（JST基準） |
| `streak_rewards` | 段階報酬。マイグレーション無しで調整でき、ユーザーにも見せる（継続の動機） |
| `user_streaks` | 現在／最長の連続日数・最終受け取り日 |
| `claim_daily_streak()` | 受け取り。連続で加算、最終段の翌日は1日目に戻る |
| `my_streak()` | 「今日受け取れるか」「次にいくらか」「途切れたか」を返す |
| `admin_streak_summary` | 今日/昨日の受け取り数・3日以上/7日到達・平均継続 |

### 不正対策として重要な2点

1. **日付判定はサーバ側**。クライアントの時計を信用すると、端末の日付を進めて
   連続ボーナスを取り放題になります。
2. **冪等キーが `streak:<user>:<サービス日付>`**。連打はもちろん、
   **`user_streaks` の行を書き換えても同じ暦日に2回は付与されません**
   （DBテストで、`last_claim_on` を偽装しても残高が増えないことを検証済み）。

BAN/凍結/**退会**は他の付与経路と同じく対象外です。
